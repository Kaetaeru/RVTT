...
return function(harness: any)
	...
	local joinOutcome = scenario:expectOutcome(harness, join, "Grand fault scenario creates an initial revision")
	if joinOutcome == nil then
		return
	end
	local current = scenario:snapshot()
	harness:expect(current.revision == initial.revision + 1, "successful command advances revision once")
	...
	if not staleRevision.ok then
		harness:equal(staleRevision.error.code, "STALE_REVISION", "stale revision has a stable error code")
	...
	harness:equal(scenario:snapshot().revision, current.revision, "stale revision does not mutate authority state")
	...
	if not staleEpoch.ok then
		...
		harness:expect(staleEpoch.error.retryable == false, "stale epoch requires a full authority refresh")
	end
	harness:equal(scenario:snapshot().revision, current.revision, "stale epoch does not mutate authority state")
	...
	if not invalidPayload.ok then
		harness:equal(invalidPayload.error.code, "VALIDATION_FAILED", "invalid payload has a stable error code")
	end
	...
	if first.ok and second.ok then
		harness:equal(second.value.revision, first.value.revision, "duplicate command does not add a revision")
		harness:equal(second.value.outcome.ready, first.value.outcome.ready, "duplicate command ignores conflicting replay payload")
	end
	...
	if not corruptRestore.ok then
		harness:equal(corruptRestore.error.code, "MIGRATION_FAILED", "corrupt restore has a stable migration error")
	end
	harness:equal(
	...
	harness:expect(restored.authorityEpoch ~= previousEpoch, "valid restore refreshes authority epoch")
	...
	if not oldEpochCommand.ok then
		harness:equal(oldEpochCommand.error.code, "STALE_EPOCH", "old epoch is rejected after restore")
	end
end
