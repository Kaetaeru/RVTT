...
return function(harness)
	...
	local accentOutcome =
		scenario:expectOutcome(harness, accent, "Slice 08 stores an approved accent palette")
	...
	if not invalidScale.ok then
		harness:equal(
			invalidScale.error.code,
			"VALIDATION_FAILED",
			"invalid UI scale uses validation error"
		)
	end
	...
	if not invalidAccent.ok then
		harness:equal(
			invalidAccent.error.code,
			"VALIDATION_FAILED",
			"invalid accent uses validation error"
		)
	end
	...
	local otherUserOutcome = scenario:expectOutcome(
		harness,
		otherUser,
		"Slice 08 stores another user's preference separately"
	)
	if otherUserOutcome == nil then
		return
	end
	harness:equal(
		otherUserOutcome.accentPaletteId,
		"silver",
		"other user receives the requested palette"
	)
	...
	if not unknownPreference.ok then
		harness:equal(
			unknownPreference.error.code,
			"VALIDATION_FAILED",
			"unknown preference uses validation error"
		)
	end
	...
	if restore.ok then
		...
		harness:expect(
			restoredPreferences["808"].reducedMotion == true,
			"restore preserves reduced motion"
		)
		harness:equal(
			restoredPreferences["808"].accentPaletteId,
			"teal",
			"restore preserves accent palette"
		)
	end
end
