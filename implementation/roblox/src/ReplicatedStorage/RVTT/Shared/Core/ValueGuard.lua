...
local function inspect(value: unknown, limits: Limits, depth: number, counter: { count: number }, seen: { [table]: boolean }): boolean
    counter.count += 1
...
function ValueGuard.isVector3Record(value: unknown, absoluteLimit: number): boolean
    ...
    for _, axis in { "x", "y", "z" } do
        ...
        if not ValueGuard.isFiniteNumber(component) or math.abs(component :: number) > absoluteLimit then
            return false
        end
    end
    return true
