...
function AuthorityRuntime:execute(context, envelope)
    ...
    local transactionResult = self.transactionCoordinator:execute(self.state, function(draft)
        ...
        return descriptor.execute(context, draftDomains[descriptor.domainId], envelope.payload, draftDomains)
    end)
...
function AuthorityRuntime:restore(document)
    if type(document) ~= "table" or document.schemaVersion ~= Version.SCHEMA or type(document.domains) ~= "table" then
