...
local function projectSession(state, viewer)
    return {
        ...
        selectedCharacter = if viewer.role == "dm"
            ...
            else { [tostring(viewer.userId)] = state.selectedCharacter[tostring(viewer.userId)] },
...
local function projectScene(state, viewer)
    ...
    for actorId, actor in state.actors do
        if viewer.role == "dm" or actor.hidden ~= true or actor.ownerUserId == viewer.userId or actor.controllerUserId == viewer.userId then
...
local function projectCharacter(state, viewer)
    ...
    for id, draft in state.drafts do
        if draft.ownerUserId == viewer.userId then drafts[id] = DeepCopy(draft) end
    end
...
local function projectInventory(state, viewer, domains)
    if viewer.role == "dm" then return DeepCopy(state) end
    local items = {}
    local locations = {}
...
local function projectExploration(state, viewer)
    ...
    for key, value in state.knowledge do
        if string.sub(key, 1, #prefix) == prefix then knowledge[key] = value end
    end
    local searches = {}
    for id, search in state.searches do
        if search.observerUserId == viewer.userId then searches[id] = DeepCopy(search) end
    end
    return {
...
local function projectJournal(state, viewer)
    ...
    for id, ping in state.pings do
        if ping.expiresAt > now then pings[id] = DeepCopy(ping) end
    end
    return { documents = documents, pings = pings }
end

function DomainProjectionPolicy.project(domainId: string, state, viewer, domains)
    if domainId == "session" then return projectSession(state, viewer) end
    if domainId == "scene" then return projectScene(state, viewer) end
    if domainId == "character" then return projectCharacter(state, viewer) end
    if domainId == "inventory" then return projectInventory(state, viewer, domains) end
    if domainId == "exploration" then return projectExploration(state, viewer) end
    if domainId == "rules" then return projectRules(state, viewer) end
    if domainId == "journal" then return projectJournal(state, viewer) end
    if domainId == "ui_preferences" then
        return { byUser = { [tostring(viewer.userId)] = DeepCopy(state.byUser[tostring(viewer.userId)] or {}) } }
    end
