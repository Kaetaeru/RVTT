...
local function profileFromCharacter(character)
    ...
    local maximum = math.max(1, 8 + constitutionModifier + (level - 1) * math.max(1, 5 + constitutionModifier))
    return {
        source = "character",
        abilities = abilities,
...
function ActorProfileResolver.resolve(actorId: string, domains)
    ...
    local character = characterDomain and characterDomain.characters[actor.sourceCharacterId or actorId]
