...
function Domain.register(registry)
    registry:register({
        ...
        validate = function(payload)
            return (payload.title == nil or (type(payload.title) == "string" and #payload.title <= 160))
                and (payload.body == nil or (type(payload.body) == "string" and #payload.body <= 20000))
        end,
    ...
    registry:register({
        ...
        authorize = function(context, domains, payload)
            ...
            return document ~= nil and (document.ownerUserId == context.playerId or context.role == "dm")
        end,
        validate = function(payload)
        ...
        execute = function(_, state, payload)
            ...
            if payload.title ~= nil then document.title = payload.title end
            if payload.body ~= nil then document.body = payload.body end
            if type(payload.links) == "table" then document.links = payload.links end
            document.revision += 1
            return document
        end,
    })
    ...
    registry:register({
        ...
        validate = function(payload)
            return Helpers.isVector(payload.position)
                and (payload.label == nil or (type(payload.label) == "string" and #payload.label <= 80))
        end,
        execute = function(context, state, payload)
