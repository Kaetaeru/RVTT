...
function DomainHelpers.hasString(payload: { [string]: unknown }, key: string, maximum: number?): boolean
...
function DomainHelpers.controlsActor(context, domains, actorId: unknown): boolean
    ...
    return actor ~= nil and (actor.controllerUserId == context.playerId or actor.ownerUserId == context.playerId)
end
...
function DomainHelpers.ownsItem(context, domains, itemId: unknown): boolean
    ...
    return location.characterId ~= nil and DomainHelpers.ownsCharacter(context, domains, location.characterId)
end

function DomainHelpers.copyFields(source: { [string]: unknown }, allowed: { [string]: boolean }): { [string]: unknown }
    local result = {}
