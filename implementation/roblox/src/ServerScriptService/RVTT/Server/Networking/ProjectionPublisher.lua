...
function ProjectionPublisher.new(runtime, builder, remotes, roleResolver)
	return setmetatable({ runtime = runtime, builder = builder, remotes = remotes, roleResolver = roleResolver }, ProjectionPublisher)
end

function ProjectionPublisher:publish(player: Player)
	local projection = self.builder:build(self.runtime:snapshot(), player.UserId, self.roleResolver(player))
