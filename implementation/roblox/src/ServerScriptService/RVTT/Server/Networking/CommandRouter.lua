...
function CommandRouter:start()
    self.remotes.command.OnServerEvent:Connect(function(player, rawEnvelope)
        ...
        if not self.rateLimiter:allow(key) then
            self.remotes.receipt:FireClient(player, receipt(nil, "rejected", Result.err(
                "RATE_LIMITED",
        ...
        self.remotes.receipt:FireClient(player, receipt(
            envelope.commandId,
            "accepted",
        ...
        self.remotes.receipt:FireClient(player, receipt(envelope.commandId, "terminal", terminalResult))
